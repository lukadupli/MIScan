...
class _TransformPageState extends State<TransformPage> {
  ...
  Widget build(BuildContext context){
    debugPrint("${fController.childSize}");
    return Scaffold(
      appBar: AppBar(
          ...
          Stack(
            children: [
              Frame(
                ...
                onDragStart: (index){
                  active.add(index);
                  if(active.length == 2) showcaseAlignment.value = null;
                },
                onPositionChange: (index) {
                  if(active.length != 1) return;
                  
                  if(showcaseAlignment.value != (fController.corners[index].dx < fController.childSize.width / 2)) showcaseAlignment.value =  fController.corners[index].dx < fController.childSize.width / 2;

                  showcasePosition.value = fController.corners[index] * ratio;
                },
                onDragEnd: (index) {
                  active.remove(index);
                  if(active.isEmpty) showcaseAlignment.value = null;
                }, 
